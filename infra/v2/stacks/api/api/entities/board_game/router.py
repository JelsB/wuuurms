...
def update_board_game(
    id: Annotated[UUID4, Path(title='Unique identifier of the board game to update')], board_game: UpdateBoardGameInput
):
    try:
        logic.update_board_game(str(id), board_game)
    except ItemNotFound:
        raise HTTPException(status_code=404, detail=f'Board game with id {id} was not found.')
    except DatabaseException:
        raise HTTPException(status_code=500, detail='Internal server error')

    return {'message': f'Board game with id {id} was successfully updated.'}
